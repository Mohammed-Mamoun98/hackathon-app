...
import { exchangeCols } from '../DexTable/cols';

export default function CexTable() {
  ...
  if (!matchedToken) return <>Tokessn Not found</>;
  ...
  return (
    <div className="flex-[1]  bg-hovered-nuetral rounded-lg p-4">
      <span className="text-3xl text-content-tirtiary font-bold">CEXs</span>
      <Skeleton
        loading={gettingInfo}
        length={1}
      ...
      >
        <Table
          ...
          cols={renderedExchangeCols}
          dataSourse={exchangeArr}
        />
      </Skeleton>
    </div>
  );
